...
def Clean(X_data,Y_data):
    ...
    Cleaner = LogisticRegression()
    Cleaner.fit(X_data,Y_data)
    ...
    for i in X_data:
        #print("Probability: ",Cleaner.predict_proba([i])[0][0])
        if(Cleaner.predict_proba([counter])[0][0]<= .58 and Y_data[counter] == 1):
         X_data =np.delete(X_data,counter,0)
         Y_data =np.delete(Y_data,counter,0)
         counter -=1
         #print("Probability: ",knn.predict_proba([i])[0][0])

        if(Cleaner.predict_proba([counter])[0][0]>=.51 and Y_data[counter]==0):
         X_data =np.delete(X_data,counter,0)
         Y_data =np.delete(Y_data,counter,0)
         #print("Probability: ",knn.predict_proba([i])[0][0])
         counter -=1
